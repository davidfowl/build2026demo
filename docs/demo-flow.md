...
| --- | --- | --- |
| Calendar UI | "This is just a planner." | Dragging a block creates an intent, not a direct write. |
| Planning intent | "The agent will update the calendar." | The agent only emits structured `CalendarPatch[]` proposals. |
| Broker policy | "So who is allowed to write?" | The broker validates ownership, etags, operation type, and confirmation policy. |
| Durable state | "Where does this coordination live?" | Aspire-managed PostgreSQL stores state, intents, proposals, decisions, and audit history. |
| Aspire graph | "This is already a distributed system." | `web`, `api`, `planner`, commands, logs, health, and relationships in one AppHost. |
| Resource commands | "How do I run and debug this reliably?" | Reset, replay, simulate conflict, trigger replanning, inspect session. |
| Foundry hosted agents | "Where should model-backed agent execution live?" | Stable services stay in ACA; stateful agent sessions move to Foundry hosted agents. |
...
| 16-24 min | Agentic interaction | Drag blocks in the UI. | The UI records intents; the planner proposes patches; the broker decides. |
| 24-31 min | Safety and durability | Pending confirmations, rejected patches, Postgres/pgweb. | The broker owns authorization, and Postgres records the durable coordination trail. |
...
> This starts like a normal scheduling feature: I drag time around and expect the app to clean up the consequences.

Drag a user-owned focus block. Let the audience see the block move, then point to the audit trail. The important reveal is that the UI did not directly mutate the calendar. It created a planning intent.

Call out the first boundary:

```text
drag gesture -> planning intent
```

### 2. Reveal the planner as a separate actor

Now explain that an agent/planner is watching the intent queue. It does not receive a calendar write token. It receives context and emits a proposal:

```text
planning intent -> planner/agent -> CalendarPatch[]
```
...
Drag an accepted meeting with attendees. It should not silently move. Show that it becomes pending confirmation.
...
This is the central agentic-app lesson: the model can reason, but the app owns authorization. The broker validates user ownership, calendar scope, operation type, etags, stale state, and allowed mutations.

### 4. Reveal durable coordination in Postgres

Open the Postgres/pgweb resource from the Aspire dashboard after showing the broker decision. Show that state is no longer a hidden local JSON file. The broker persists the current calendar state plus intents, proposals, decisions, and audit entries in an Aspire-managed PostgreSQL database.

Use this line:

> Agentic coordination needs durable state: what the user asked for, what the planner proposed, what policy decided, and what actually changed.
...
| --- | --- |
| `web` | Direct manipulation calendar UI. |
| `api` | Calendar broker, fake calendar provider, policy, audit trail, SSE. |
| `postgres` / `calendar` | Durable app state for calendar events, intents, proposals, decisions, and audit history. |
| `pgweb` | Inspectable Postgres UI from the dashboard. |
| `planner` | Background planner/agent worker that proposes patches. |
...
Frame these commands strictly as local inner-loop helpers. They are not the product surface and they are not the production agent runtime. They make development and rehearsal fast because the app model carries repeatable operations for reset, replay, conflict simulation, and inspection.
...
aspire resource api trigger-replanning --apphost ./apphost.mts
aspire resource api replay-last-drag --apphost ./apphost.mts
aspire resource api simulate-calendar-conflict --apphost ./apphost.mts
```

| Command | CLI name | Inner-loop reason |
| --- | --- | --- |
| Seed demo calendar | `seed-demo-calendar` | Return Postgres to the seeded 5-event scenario. |
| Trigger replanning | `trigger-replanning` | Exercise the planner without manual UI setup. |
| Replay last drag | `replay-last-drag` | Reproduce the same intent while iterating on policy or UI. |
| Simulate stale etag conflict | `simulate-calendar-conflict` | Reproduce the conflict path on demand. |
| Clear pending patches | `clear-pending-agent-patches` | Recover the local dev state without editing data by hand. |
| Reset user calendar | `reset-user-calendar` | Hard reset to a known state. |
...
3. Drag a focus/task block and show auto-apply with undo.
4. Drag a meeting and show confirmation.
5. Reject once, then replay/drag again and approve.
6. Simulate stale etag conflict and show rejection.
7. Open Aspire dashboard and reveal `web`, `api`, `planner`, and Postgres.
8. Open pgweb to show durable coordination state.
9. Search logs/traces for the intent or proposal id.
10. Run one inner-loop command from the dashboard, then the same class of command from the CLI with `aspire resource api ...`.
11. Show `apphost.mts` as the code-first model that defines the app resources and local development command surface.
12. Preview Foundry hosted-agent mode.
13. Preview ACA deployment steps.
...
| Postgres | It matters more after the audience sees intents, proposals, and decisions. |
